...
function shoreditch_entry_footer() {
	if ( 'post' === get_post_type() ) {
		if (is_single()) {
			$time_string = '<time class="entry-date published updated helveneue-medium" datetime="%1$s">%2$s</time>';
		} else {
			$time_string = '<time class="entry-date published updated helveneue-medium" datetime="%1$s">%2$s</time>';
		}
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			$time_string = '<time class="entry-date published helveneue-medium" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
		}
		...
		if (is_single()) {
			$posted_on = sprintf( '<a rel="bookmark"><span class="helveneue-medium">PUBLICADO EL</span> %2$s</a>', esc_url( get_permalink() ), $time_string );
		} else {
			$posted_on = sprintf( '<a rel="bookmark"><span class="helveneue-medium">PUBLICADO EL</span> %2$s</a>', esc_url( get_permalink() ), $time_string );
		}
